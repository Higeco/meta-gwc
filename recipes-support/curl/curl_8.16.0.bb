...
SRC_URI = " \
    ...
    file://0001-Comaptibility_dunfell_libs.patch \
"
...
PACKAGECONFIG ??= "${COMMON_PACKAGECONFIG} ${@bb.utils.filter('DISTRO_FEATURES', 'ipv6', d)} gnutls libidn smtp websockets"
PACKAGECONFIG_class-native = "${COMMON_PACKAGECONFIG} ipv6"
PACKAGECONFIG_class-nativesdk = "${COMMON_PACKAGECONFIG} ipv6"
...
DEPENDS_append_class-target = "${@bb.utils.contains('PTEST_ENABLED', '1', ' openssl-native', '', d)}"
...
FILES_${PN}_append_class-nativesdk = " ${SDKPATHNATIVE}/environment-setup.d/curl.sh"
